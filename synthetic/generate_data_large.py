...
def get_args():
    ...
    parser.add_argument("--data_root_dir", type=str, default="./data_large")
...
def generate_samples_from_graph(G, start_token_id, num_samples):        
    edges = []
    weights = []
    ...
    sample_input_idss = [generate_sample_input_ids(edge, start_token_id) for edge in sample_edges]
    
    return sample_input_idss
...
def generate_sample_input_ids(edge, start_token_id, cls_token_id=101, sep_token_id=102):
    shuffled_edge = random.sample(edge, len(edge))
    sample_input_ids = [cls_token_id] + [int(node) + start_token_id for node in shuffled_edge] + [sep_token_id]
...
if __name__ == "__main__":
    ...
    for i in range(args.num_trials):
        ...
        sample_input_idss = generate_samples_from_graph(G, start_token_id=START_TOKEN_ID, num_samples=args.num_samples)
        # pdb.set_trace()
        torch.save(sample_input_idss, data_path)