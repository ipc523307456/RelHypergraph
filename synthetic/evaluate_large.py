...
def get_probability_from_model(prefix_token_ids, tokenizer, model, node_ids, args, exclude_node_ids=None):   
    
    token_ids = torch.cat([101 * torch.ones((prefix_token_ids.shape[0], 1), dtype=torch.long), prefix_token_ids.view(-1, 1), 
        103 * torch.ones((prefix_token_ids.shape[0], 1), dtype=torch.long), 102 * torch.ones((prefix_token_ids.shape[0], 1), dtype=torch.long)], dim=1).to("cuda")
...
def bfs_weight_estimation(relative_weight_matrix, mask):
    ...
    while not Q.empty():
        ...
        for j in range(n): 
            ...
            # Judge if visited
            if visited[a][b] > precision:
                continue
            else:
                visited[a][b] = 1
            
            if mask[a][b] > precision:
...
def estimate_graph(tokenizer, model, node_ids, args):
    ...
    for i in tqdm(range(num_nodes)):
        ...
        edge_prob = get_probability_from_model(
            prefix_token_ids=torch.tensor([node_id]),
            tokenizer=tokenizer,
            model=model,
            node_ids=node_ids,
            args=args,
    ...
    edge_probs_filter = edge_probs / np.max(edge_probs, axis=1).reshape((-1,1))
    
    edge_probs_mask = deepcopy(edge_probs)
    edge_probs_mask[edge_probs_filter < threshold] = 0
    edge_probs_mask[edge_probs_filter > threshold] = 1
...
def get_args():
    ...
    # Eval Results
    parser.add_argument("--eval_result_dir", type=str, default="./eval_results_large")
    
    parser.add_argument("--savefig_root_dir", type=str, default="./figures_large")
...
if __name__ == "__main__":    
    ...
    if os.path.exists(args.eval_result_path):
        ...
        G_adj_matrix_filter =  weighted_G_adj_matrix / np.max(weighted_G_adj_matrix, axis=1).reshape((-1,1))

        G_adj_matrix[G_adj_matrix_filter < threshold] = 0
        G_adj_matrix[G_adj_matrix_filter > threshold] = 1
