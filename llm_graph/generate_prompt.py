...
def generate_prompt_direct(concepts, args):
    ...
    prompt = prompt + ".\n"
    prompt = prompt + "Suppose that these concepts are nodes of an undirected graph.\n" 
    prompt = prompt + "For each concept, consider {} most related concepts.\n".format(args.max_width)
    prompt = prompt + "According to the relations between these concepts, which edges should be included? Please answer with an edgelist."
    print(prompt)
